...
public class WeChatParamCheck implements MessageLink<SendTaskDto> {
    ...
    public void process(LinkContext<SendTaskDto> context) {
        ...
        if (StrUtil.isBlank(pushSubject) || StrUtil.isBlank(wechatUserIdType)) {
            throw new MessageException("企微 pushSubject 或者 wechatUserIdType 参数为空");
        }
        if (!("app".equals(pushSubject) || "robot".equals(pushSubject))) {
            throw new MessageException("企微 pushSubject 非法，必须为 app 或者 robot");
        }
        if ("app".equals(pushSubject)) {
            if (!(WECHAT_APP_USER_ID_TYPE_SET.contains(wechatUserIdType)
                ...
                throw new MessageException("企微 pushSubject 为 app 时，wechatUserIdType 参数必须为 touser 或者 toparty 或者 totag 或者 to_parent_userid 或者 to_student_userid 或者 to_party 或者 toall 或者 chatid");
            }

            if (WECHAT_SCHOOL_APP_USER_ID_TYPE_SET.contains(wechatUserIdType)) {
                if (!SCHOOL_APP_MESSAGE_TYPE.contains(messageType)) {
                    throw new MessageException("当 wechatUserIdType 为 to_parent_userid 或者 to_student_userid 或者 to_party 或者 toall 时，不支持此消息类型");
                }
            } else {
                if (!CHAT_APP_MESSAGE_TYPE.contains(messageType)) {
                    throw new MessageException("当 wechatUserIdType 为 chatid 时，不支持此消息类型");
                }
            }
        } else {
            if (!"webhook".equals(wechatUserIdType)) {
                throw new MessageException("企微 pushSubject 为 robot 时，wechatUserIdType 参数必须为 webhook");
            }

            if (!ROBOT_MESSAGE_TYPE.contains(messageType)) {
                throw new MessageException("当 wechatUserIdType 为 webhook 时，不支持此消息类型");
            }
        }
    }
}
