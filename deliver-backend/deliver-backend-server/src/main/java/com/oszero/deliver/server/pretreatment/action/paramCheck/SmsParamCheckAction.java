...
import cn.dianhun.om.message.constant.SmsParamConstant;
...
@Service
public class SmsParamCheckAction implements BusinessProcess<SendTaskDto> {
    @Override
    public void process(ProcessContext<SendTaskDto> context) {
        Map<String,String> paramMap = context.getProcessModel().getParamMap();
        CommonUtil.paramCheckAndThrow(paramMap,SmsParamConstant.SMSTEXT);
    }
}
