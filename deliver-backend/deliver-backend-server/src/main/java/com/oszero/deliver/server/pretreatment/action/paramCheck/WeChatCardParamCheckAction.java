 ...
 */
@Service
public class WeChatCardParamCheckAction implements BusinessProcess<SendTaskDto> {
    @Override
    public void process(ProcessContext<SendTaskDto> context) {
        Map<String, String> paramMap = context.getProcessModel().getParamMap();
        CommonUtil.paramCheckAndThrow(paramMap,WeChatParamConstant.CARD_DES);
        CommonUtil.paramCheckAndThrow(paramMap,WeChatParamConstant.CARD_BTNTXT);
        CommonUtil.paramCheckAndThrow(paramMap,WeChatParamConstant.CARD_TITLE);
        CommonUtil.paramCheckAndThrow(paramMap,WeChatParamConstant.CARD_URL);
    }
}
